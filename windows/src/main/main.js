...
const { resolveCharacter } = require('./character-resolver');
const { resolveSocialProfile } = require('./social-profiles');
...
if (process.platform === 'win32') {
  app.setAppUserModelId('com.xiaolin.resident');
}
...
const SAD_COMFORT_INTERVAL_MS = 9000;
const SAD_COMFORT_MESSAGES = [
  '有我喺度',
  '唔使急住好返，我陪住你',
  '今日攰咗都冇問題',
  '慢慢嚟，想抖幾耐都得',
...
let socialStateTimer = null;
let sadComfortIndex = 0;
let sadComfortTimer = null;
...
function createMainWindow({ showOnReady = true } = {}) {
  mainWindow = new BrowserWindow({
    ...
    icon: path.join(__dirname, '..', 'renderer', 'assets', 'character-states', 'default.png'),
  ...
  petWindow.webContents.on('context-menu', () => {
    const template = [
      { label: '開啟關懷中心', click: showMainWindow },
      {
        label: '快速切換狀態',
        submenu: [
          { label: '休息', type: 'radio', checked: careConfig?.currentState === 'leisure', click: () => setPetManualState('leisure') },
          { label: '工作', type: 'radio', checked: careConfig?.currentState === 'working', click: () => setPetManualState('working') },
          { label: '傷心', type: 'radio', checked: careConfig?.currentState === 'sad', click: () => setPetManualState('sad') },
          { label: '摸魚', type: 'radio', checked: careConfig?.currentState === 'slacking', click: () => setPetManualState('slacking') },
...
function setSocialState(state, durationMs = 0) {
  clearTimeout(socialStateTimer);
  socialStateTimer = null;
  socialState = state;
  broadcastState();

  if (durationMs > 0) {
    socialStateTimer = setTimeout(() => {
      socialStateTimer = null;
      socialState = null;
      broadcastState();
    }, durationMs);
...
function syncSadComfortLoop() {
  if (careConfig?.currentState !== 'sad') {
    clearInterval(sadComfortTimer);
    sadComfortTimer = null;
    sadComfortIndex = 0;
    return;
  }

  if (sadComfortTimer) return;
  sadComfortIndex = 0;
  sadComfortTimer = setInterval(() => {
    if (careConfig?.currentState !== 'sad') {
      syncSadComfortLoop();
      return;
    }
    sadComfortIndex = (sadComfortIndex + 1) % SAD_COMFORT_MESSAGES.length;
    broadcastState();
  }, SAD_COMFORT_INTERVAL_MS);
}

function advanceSadComfort() {
  if (careConfig?.currentState !== 'sad') return buildUnifiedState();
  sadComfortIndex = (sadComfortIndex + 1) % SAD_COMFORT_MESSAGES.length;
  broadcastState();
...
function getPetLayoutMode() {
  ...
  if (careConfig?.currentState === 'sad') return 'comfort';
  return 'daily';
}
...
function buildUnifiedState() {
  ...
  if (character.stateKey === 'sad') {
    statusText = SAD_COMFORT_MESSAGES[sadComfortIndex] || character.statusText;
  }
  if (activeReminder) {
...
function registerIpcHandlers() {
  ...
  // 動態巡邏站：只允许打开预设的 Instagram / Threads 官方主页。
  ipcMain.handle('social:open', async (_event, platform) => {
    const url = resolveSocialProfile(platform);
    if (!url) return false;
    setSocialState('patrolling', 8000);
    try {
  ...
  ipcMain.handle('care:comfort-next', async () => advanceSadComfort());
  ...
  try {
    ...
    const mainState = await mainWindow.webContents.executeJavaScript(`(() => {
      return {
        ...
        hasPatrolStation: Boolean(document.getElementById('patrolStation')),
        hasInstagramButton: Boolean(document.getElementById('openInstagramButton')),
        hasThreadsButton: Boolean(document.getElementById('openThreadsButton')),
        socialPlatformCards: document.querySelectorAll('.social-platform-card').length,
        hasNoDemoMaterials: !document.getElementById('materialList'),
        ...
        hasTabPatrol: Boolean(document.getElementById('tabBtn-patrol')),
    ...
    const sadComfortTexts = [];
    let sadMainCapturePath = null;
    let sadPetCapturePath = null;
    for (const [stateKey, expectedFile] of [
      ...
      if (stateKey === 'sad') {
        const sadBefore = await petWindow.webContents.executeJavaScript(`(() => ({
          text: document.getElementById('petStatus')?.textContent,
          animation: getComputedStyle(document.getElementById('petCharacter')).animationName
        }))()`);
        if (sadBefore.animation !== 'pet-comfort') {
          throw new Error(`伤心状态没有启用安慰动画，实际为 ${sadBefore.animation}`);
        }
        sadComfortTexts.push(sadBefore.text);
        await petWindow.webContents.executeJavaScript(`window.xiaolinPet.comfortNext()`);
        await new Promise((resolve) => setTimeout(resolve, 80));
        const sadAfter = await petWindow.webContents.executeJavaScript(
          `document.getElementById('petStatus')?.textContent`
        );
        sadComfortTexts.push(sadAfter);
        if (!sadAfter || sadAfter === sadBefore.text) {
          throw new Error('点击伤心状态后，安慰文字没有切换。');
        }

        const extension = path.extname(capturePath) || '.png';
        sadMainCapturePath = path.join(
          path.dirname(capturePath),
          `${path.basename(capturePath, path.extname(capturePath))}-sad${extension}`
        );
        sadPetCapturePath = path.join(
          path.dirname(capturePath),
          `${path.basename(capturePath, path.extname(capturePath))}-pet-sad${extension}`
        );
        await fsPromises.writeFile(sadMainCapturePath, (await mainWindow.webContents.capturePage()).toPNG());
        await fsPromises.writeFile(sadPetCapturePath, (await petWindow.webContents.capturePage()).toPNG());
      }
    }

    const socialStateImages = {};
    careConfig = await careStore.patch({ currentState: 'leisure' });
    for (const [stateKey, expectedFile] of [
      ['patrolling', 'patrolling.png'],
      ['collecting', 'collecting.png']
    ]) {
      setSocialState(stateKey);
      await new Promise((resolve) => setTimeout(resolve, 80));
      const actualSrc = await petWindow.webContents.executeJavaScript(
        `document.querySelector('.pet-character img')?.getAttribute('src')`
      );
      socialStateImages[stateKey] = actualSrc;
      if (!actualSrc?.endsWith(expectedFile)) {
        throw new Error(`${stateKey} 状态没有加载 ${expectedFile}，实际为 ${actualSrc}`);
      }
    }
    ...
    console.log(`XIAOLIN_SMOKE_RESULT=${JSON.stringify({ ...mainState, ...petState, manualStateImages, socialStateImages, sadComfortTexts })}`);
    console.log(`XIAOLIN_SMOKE_MAIN_CAPTURE=${capturePath}`);
    console.log(`XIAOLIN_SMOKE_PET_CAPTURE=${petCapturePath}`);
    if (sadMainCapturePath) console.log(`XIAOLIN_SMOKE_SAD_CAPTURE=${sadMainCapturePath}`);
    if (sadPetCapturePath) console.log(`XIAOLIN_SMOKE_PET_SAD_CAPTURE=${sadPetCapturePath}`);
  } catch (error) {
...
app.whenReady().then(async () => {
  ...
  createPetWindow();
  createTray();
  if (!process.env.XIAOLIN_SMOKE_CAPTURE) setupSleepDetection();
  // 動態巡邏站是主窗口的核心入口，启动后直接显示关怀中心。
