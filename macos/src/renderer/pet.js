(function startPetPlayer() {
  ...
  let currentStateKey = 'leisure';
  let comfortClickTimer = null;
  characterBox.addEventListener('click', () => {
    if (currentStateKey !== 'sad') return;
    clearTimeout(comfortClickTimer);
    comfortClickTimer = setTimeout(requestComfort, 240);
  });
  characterBox.addEventListener('dblclick', () => {
    clearTimeout(comfortClickTimer);
    desktop.openMain();
  });
  ...
  async function requestComfort() {
    if (currentStateKey !== 'sad' || !desktop?.comfortNext) return;
    characterBox.classList.remove('comfort-pulse');
    void characterBox.offsetWidth;
    characterBox.classList.add('comfort-pulse');
    window.setTimeout(() => characterBox.classList.remove('comfort-pulse'), 950);
    try {
      await desktop.comfortNext();
    } catch {
      // 保持安静，不让互动错误破坏陪伴感。
    }
  }
