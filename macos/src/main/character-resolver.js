 ...
 * P0 实现 do_not_disturb / reminder_drink / working / leisure / sad / slacking / happy；
 ...
 *
 * @param {object} input
 * @param {string} input.manualState 用户手动选择的状态：working / leisure / sad / slacking / do_not_disturb
 ...
 * @param {string|null} [input.socialState] 动态巡逻状态：patrolling / collecting
...
function resolveCharacter(input = {}) {
  ...
  } else if (manualState === 'do_not_disturb') {
    stateKey = 'do_not_disturb';
  } else if (socialState === 'patrolling' || socialState === 'collecting') {
    stateKey = socialState;
  } else if (happyActive) {
    stateKey = 'happy';
  } else if (manualState === 'working' || manualState === 'sad' || manualState === 'slacking') {
