...
test('patch 接受伤心与摸鱼状态', async (context) => {
  const store = await makeStore(context);
  await store.patch({ currentState: 'sad' });
  assert.equal((await store.getAll()).currentState, 'sad');
  await store.patch({ currentState: 'slacking' });
  assert.equal((await store.getAll()).currentState, 'slacking');
});
